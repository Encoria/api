...
public class PublicationController {
    ...
    @PostMapping("/{publicationUuid}/comments")
    public ResponseEntity<PublicationCommentResponse> addPublicationComment(@AuthenticationPrincipal Jwt jwt,
                                                                            @PathVariable UUID publicationUuid,
                                                                            @RequestBody String content) {
        return new ResponseEntity<>(
                publicationService.commentOnPublication(jwt, publicationUuid, content),
                HttpStatus.CREATED);
    }

    @DeleteMapping("/{publicationUuid}/comments/{commentUuid}")
    public ResponseEntity<Void> deleteComment(@AuthenticationPrincipal Jwt jwt,
                                              @PathVariable UUID publicationUuid,
                                              @PathVariable UUID commentUuid) {
        publicationService.deleteComment(jwt, publicationUuid, commentUuid);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
